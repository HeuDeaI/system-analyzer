...
func NewConcurrencyPanel() fyne.CanvasObject {
	...
	tests := []struct {
		...
		{"Накладные расходы на горутины", "Измерение временных затрат на создание и завершение легковесных потоков (горутин).", concurrency.GoroutineOverheadBenchmark, widget.NewLabel("Результат: ...")},
		{"Блокировка мьютекса", "Оценка производительности примитивов синхронизации (sync.Mutex) в условиях конкуренции.", concurrency.MutexBenchmark, widget.NewLabel("Результат: ...")},
		{"Атомарные операции", "Оценка производительности атомарных операций как альтернативы блокировкам.", concurrency.AtomicBenchmark, widget.NewLabel("Результат: ...")},
		{"Пропускная способность каналов", "Измерение скорости передачи сообщений через типизированные каналы.", concurrency.ChannelBenchmark, widget.NewLabel("Результат: ...")},
		{"Конвейерная обработка", "Оценка эффективности многостадийной обработки данных в параллельных потоках.", concurrency.PipelineBenchmark, widget.NewLabel("Результат: ...")},
	}
